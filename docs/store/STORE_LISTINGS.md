# Awwad - Store listings (ASO) - ar / en / fr

> Generated + adversarially verified 2026-07-04 (character limits checked programmatically,
> MSA audit, policy-safety audit). Paste these into Google Play Console / App Store Connect
> as-is. Arabic is the DEFAULT store language; add English and French as localizations.
>
> Live URLs used inside the copy:
> - Website: https://awwad-habits.netlify.app
> - Privacy: https://awwad-habits.netlify.app/privacy
> - Delete account: https://awwad-habits.netlify.app/delete-account
> - Web app: https://awwad-app.netlify.app
>
> After each app is PUBLISHED: set `androidLive`/`iosLive` to `true` (and the real iOS id)
> in `web/src/content/site.js`, rebuild the site and redeploy.

---

## العربية (اللغة الافتراضية)

### Google Play
- **اسم التطبيق** (26/30):
  `عوّاد: ترك العادات وبناؤها`
- **الوصف المختصر** (74/80):
  `اترك عادة سيئة وابنِ عادة نافعة: تطبيق عربي مجاني لتطوير الذات بلا إعلانات`
- **الوصف الكامل** (2234/4000):

```
عوّاد رفيقك العربي لترك العادات السيئة وبناء العادات النافعة، بأسلوب علمي عملي وروح داعمة دافئة.

«رفيقُ مَن زانَ عُمرَهُ، وحَسُنَ عملُهُ»

هل تريد ترك التدخين، أو التوقف عن السهر، أو الإقلاع عن العادة السرية؟ أم تسعى إلى عادات يومية نافعة مثل المحافظة على الصلاة وورد القرآن؟ عوّاد يرافقك خطوة بخطوة، مجاناً وبلا إعلانات.

🎯 مساران لتغيير حياتك
• مسار "اترك عادة": التدخين، قضم الأظافر، التسويف، إدمان الهاتف، السهر، المشاهدة المفرطة، الغضب، العادة السرية، نتف الشعر وغيرها
• مسار "ابنِ عادة": الصلاة في وقتها، الاستيقاظ لصلاة الفجر، الورد اليومي من القرآن، الصلاة على النبي، بر الوالدين، الدعاء، أذكار الصباح والمساء، الرياضة، القراءة، شرب الماء، النوم المبكر، الحمد والشكر، الصيام التطوعي
• قوالب جاهزة: 36 عادة معدة مسبقاً، مع إمكانية إنشاء عادات مخصصة
• تابع حتى 3 عادات للترك و3 عادات للبناء معاً، مع مبدّل سريع بين العادات

🧭 منهج علمي واضح
يعتمد عوّاد على أسلوب التدريب على عكس العادة (Habit Reversal Training) في أربع مراحل:
• الوعي: تعرّف على محفزات عادتك ولحظات حدوثها
• الاستجابة البديلة: استبدل بالسلوك القديم سلوكاً نافعاً
• ضبط البيئة: أبعد المثيرات وهيئ محيطك للنجاح
• التثبيت: حافظ على مكاسبك وحوّلها إلى نمط حياة دائم

🛠 أدوات تساعدك كل يوم
• سجل يومي ذكي: مقاييس خاصة بكل عادة، وتدوين الحالة المزاجية والملاحظات، وقوائم متابعة مصممة وفق المنهج
• إحصاءات ورسوم بيانية توضح تقدمك بمرور الأيام
• أوسمة تحفيزية ودروع مواظبة: درع فضي بعد 30 يوماً، وذهبي بعد 60، وماسي بعد 90
• مؤقت بومودورو للتركيز وتنظيم الوقت
• تذكيرات يومية متعددة قابلة للتخصيص لكل عادة
• إشعار يومي اختياري بذكر صحيح: الصلاة الإبراهيمية كما وردت في صحيح مسلم
• سجل كامل يحفظ تاريخ التزامك

🤍 قيم نعتز بها
• تطبيق متوافق مع القيم الإسلامية، بلغة داعمة من غير وعظ
• مجاني بالكامل: لا إعلانات، ولا اشتراكات، ولا ميزات مدفوعة
• الخصوصية أولاً: بياناتك تبقى على جهازك، والحساب اختياري للمزامنة السحابية المجانية
• يعمل دون اتصال بالإنترنت بشكل كامل
• ثلاث لغات: العربية (الافتراضية) والإنجليزية والفرنسية، مع وضع داكن مريح للعين

ملاحظة: عوّاد أداة دعم سلوكي تساعدك على تنظيم جهدك في تغيير العادات، وهو ليس بديلاً عن الاستشارة الطبية أو النفسية المتخصصة.

ابدأ اليوم خطوتك الأولى في تطوير الذات: حمّل عوّاد الآن، فكل يوم جديد فرصة للعودة إلى ما هو أجمل.

الموقع: https://awwad-habits.netlify.app
سياسة الخصوصية: https://awwad-habits.netlify.app/privacy
```

### App Store (iOS)
- **الاسم** (26/30): `عوّاد: ترك العادات وبناؤها`
- **العنوان الفرعي** (27/30): `تتبع عاداتك اليومية وأذكارك`
- **حقل الكلمات المفتاحية** (98/100):
  `تطوير,الذات,سهر,التدخين,تنظيم,الوقت,صلاة,الفجر,قرآن,إقلاع,عادة,سرية,انضباط,تحفيز,بومودورو,نوم,مبكر`
- **النص الترويجي** (149/170):
  `«رفيقُ مَن زانَ عُمرَهُ، وحَسُنَ عملُهُ» - عوّاد يرافقك خطوة بخطوة لترك عادة سيئة أو بناء عادة نافعة، مجاناً وبلا إعلانات، ويعمل دون اتصال بالإنترنت.`
- **الوصف**: استخدم نص «الوصف الكامل» أعلاه نفسه.

### استهداف الكلمات (ملخص)
يستهدف الإدراج العربي: «ترك العادات»، «تطوير الذات»، «ترك التدخين»، «العادة السرية»، «عادات يومية»، «أذكار»، «صلاة الفجر»، «ورد القرآن»، «تنظيم الوقت»، «بومودورو»، «سهر»، «نوم مبكر». حقل كلمات iOS خالٍ من تكرار كلمات الاسم والعنوان الفرعي (آبل تتجاهل المكرر).

---

## English

### Google Play
- **Title** (27/30): `Awwad: Muslim Habit Tracker`
- **Short description** (77/80):
  `Break bad habits, build good ones: prayer, Quran, quit smoking. Free, no ads.`
- **Full description** (~2300/4000):

```
Awwad is a free habit tracker with two clear paths: break a bad habit or build a good one. Arabic-first, fully available in English and French, and shaped by Islamic values: warm, supportive, never preachy.

TWO TRACKS, ONE GOAL
• Break a bad habit: quit smoking, nail biting, procrastination, phone addiction, staying up late, binge watching, anger and more
• Build a good habit: prayer on time, waking up for Fajr, a daily Quran portion, salawat on the Prophet, morning and evening adhkar, dhikr and dua, honoring parents, exercise, reading, drinking water, sleeping early, gratitude, voluntary fasting
• 36 ready-made habit templates, plus fully custom habits
• Work on up to 3 break and 3 build habits at once, with a quick habit switcher

A REAL METHOD, NOT JUST CHECKBOXES
Awwad walks you through Habit Reversal Training (HRT), a structured behavioral approach in four stages:
• Awareness: learn to notice your triggers and urges
• Competing response: replace the old habit with a better action
• Environment control: reshape your surroundings to make change easier
• Maintenance: protect your progress for the long run

EVERYTHING YOU NEED TO STAY ON TRACK
• Daily log with smart per-habit metrics, mood and notes
• Tailored HRT checklists for every habit, every day
• Statistics and clear progress charts
• Motivational badges and streak shields: silver at 30 days, gold at 60, diamond at 90
• Full history of your journey
• Pomodoro focus timer for deep work, study and time management
• Multiple daily reminders per habit
• Optional daily authentic dhikr notification: the Ibrahimic salawat, narrated in Sahih Muslim

PRIVATE, FREE AND YOURS
• 100% free: no ads, no subscriptions, no locked features
• Works fully offline; your data stays on your device
• Optional free account for cloud sync across your devices
• Arabic (default), English and French
• Comfortable dark theme

Whether you searched for a habit tracker, a way to quit smoking, a prayer and adhkar companion, or a self improvement app that respects your values, Awwad (عوّاد) was built for you.

Awwad is a behavioral support tool. It is not medical advice and does not replace professional care.

Start today: choose one habit to break or one to build, and let Awwad walk with you, one day at a time.

Website: https://awwad-habits.netlify.app
Privacy: https://awwad-habits.netlify.app/privacy
```

### App Store (iOS)
- **Name** (27/30): `Awwad: Muslim Habit Tracker`
- **Subtitle** (27/30): `Quit smoking. Pray on time.`
- **Keywords** (95/100):
  `quran,fajr,adhkar,dhikr,salah,streak,routine,islamic,focus,pomodoro,discipline,goals,ramadan,dua`
- **Promotional text** (163/170):
  `Two paths: break a bad habit or build a good one. 36 ready templates, streaks, reminders and a Pomodoro timer. 100% free, no ads, works offline, private by design.`
- **Description**: use the Play full description above.

---

## Français

### Google Play
- **Titre** (27/30): `Awwad : Tracker d'habitudes`
- **Description courte** (79/80):
  `Brisez une mauvaise habitude, bâtissez une bonne. Gratuit, sans pub, hors ligne`
- **Description complète** (2654/4000):

```
Awwad (عوّاد) est votre compagnon gratuit pour changer vos habitudes, un jour à la fois. Rompez avec une mauvaise habitude ou construisez une bonne routine, avec une méthode claire et un accompagnement chaleureux, aligné sur les valeurs musulmanes.

«رفيقُ مَن زانَ عُمرَهُ، وحَسُنَ عملُهُ»

DEUX PARCOURS, UN SEUL OBJECTIF
• Rompre une mauvaise habitude : arrêter de fumer, se ronger les ongles, procrastination, dépendance au téléphone, veillées tardives, visionnage compulsif, colère et plus encore
• Construire une bonne habitude : prière à l'heure, réveil pour le Fajr, lecture quotidienne du Coran, salawat sur le Prophète, adhkar du matin et du soir, invocations, bienfaisance envers les parents, sport, lecture, hydratation, coucher tôt, gratitude, jeûne surérogatoire
• 36 modèles d'habitudes prêts à l'emploi, plus vos habitudes personnalisées
• Jusqu'à 3 habitudes à rompre et 3 à construire en même temps, avec un sélecteur rapide

UNE MÉTHODE STRUCTURÉE : LE RENVERSEMENT D'HABITUDES (HRT)
• Prise de conscience : identifiez vos déclencheurs
• Réponse concurrente : remplacez le geste automatique par une action choisie
• Contrôle de l'environnement : éloignez les tentations
• Maintien : consolidez vos acquis jour après jour
Awwad est un outil de soutien comportemental ; il ne remplace pas un avis médical.

DES OUTILS QUI VOUS SUIVENT AU QUOTIDIEN
• Journal quotidien : indicateurs intelligents par habitude, humeur, notes et listes HRT adaptées
• Statistiques et graphiques pour visualiser vos progrès
• Badges de motivation et boucliers : argent (30 jours), or (60 jours), diamant (90 jours)
• Historique complet de votre parcours
• Minuteur Pomodoro pour vos séances de concentration
• Plusieurs rappels quotidiens par habitude
• Notification quotidienne optionnelle d'un dhikr authentique (prière ibrahimique, Sahih Muslim)

VOS VALEURS, VOTRE LANGUE, VOTRE VIE PRIVÉE
• 100 % gratuit : pas de publicité, pas d'achats intégrés, pas de fonctions payantes
• Fonctionne entièrement hors ligne ; compte gratuit optionnel pour la synchronisation cloud
• Vie privée d'abord : vos données restent sur votre appareil, sauf si vous créez un compte
• Trois langues : arabe (par défaut), français, anglais
• Thème sombre élégant
• Un ton bienveillant et digne, jamais moralisateur

Que vous souhaitiez arrêter de fumer, vous lever pour le Fajr, lire le Coran chaque jour ou simplement mieux organiser votre temps, Awwad marche à vos côtés, étape par étape.

Téléchargez Awwad aujourd'hui et faites de votre prochaine bonne habitude une réalité.

Site : https://awwad-habits.netlify.app
Confidentialité : https://awwad-habits.netlify.app/privacy
```

### App Store (iOS)
- **Nom** (27/30): `Awwad : Tracker d'habitudes`
- **Sous-titre** (29/30): `Rompez, bâtissez vos routines`
- **Mots-clés** (96/100):
  `fumer,arrêter,prière,coran,adhkar,islam,musulman,motivation,discipline,pomodoro,fajr,dhikr,suivi`
- **Texte promotionnel** (162/170):
  `Awwad vous accompagne pour rompre une mauvaise habitude ou en bâtir une bonne : méthode HRT, rappels, statistiques et badges. 100 % gratuit, sans pub, hors ligne.`
- **Description** : utilisez la description complète Play ci-dessus.
